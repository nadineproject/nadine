...
<table class="bill-fees">
	<tr>
		<th>Date:</th>
		<td>
			<a href="{% url 'staff.views.bill' bill.id %}">{{ bill.bill_date|date:"m/d/y" }}</a>
	...
	<tr><th>Status:</th>
		<td>
			{% if bill.transactions.all %}
				closed on {% for transaction in bill.transactions.all %}<a href="{% url 'staff.views.transaction' transaction.id %}">{{ transaction.transaction_date | date:"m/d/y"}}</a>{% endfor %}
			{% else %}
				open
			{% endif %}
		</td>
