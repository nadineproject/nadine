...
<table class="bill-fees">
	<tr>
		...
		<td>
			...
			{% if show_member_name %}for <a href="{% url 'staff:user_detail' bill.user.username %}">{{ bill.user.get_full_name }}</a>{% endif %}
		</td>
	</tr>
	<tr><th>Status:</th>
		<td>
	...
	{% if not hide_paid_by %}
		{% if bill.paid_by %}
			<tr>
				<th>Paid by:</th><td><a href="{% url 'staff:user_detail' bill.paid_by.user.username %}">{{ bill.paid_by.user.get_full_name }}</a></td>
			</tr>
		{% endif %}
	{% endif %}
	{% if bill.membership %}
	...
	<tr>
		...
		<td>
			{% for dropin in bill.guest_dropins.all %}
				<a href="{% url 'staff:user_detail' dropin.user.username %}">{{dropin.user.get_full_name}}</a>
