...
class Migration(DataMigration):
	...
	def forwards(self, orm):
		...
		for member in orm.Member.objects.all():
			if member.notes:
				note = orm.MemberNote.objects.create(member=member, note=member.notes)
				note.created = note.member.user.date_joined
				note.save()
				
		"Membership Notes Field"
		for membership in orm.Membership.objects.all():
			if membership.note:
				note = orm.MemberNote.objects.create(member=membership.member, note=membership.note)
				note.created = membership.start_date
				note.save()
	...
	models = {
		...
		u'staff.dailylog': {
			...
			'created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 6, 23, 0, 0)', 'auto_now_add': 'True', 'blank': 'True'}),
