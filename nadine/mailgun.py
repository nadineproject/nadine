...
def staff(request):
	...
	mailgun_data["bcc"] = User.objects.filter(is_staff=True, is_active=True).values_list('email')
...
def team(request):
	...
	mailgun_data["bcc"] = Member.objects.managers(include_future=True).values_list('user__email')
